...
class MVTRemoteRunner:
    ...
    def run(self, suite, timeout=1800):
        ...
        if suite in MVT_EXTENSION_TESTS:
            return
        self._num_of_tests = int(self.webdriver.driver.execute_script("return globalRunner.testList.length"))
        self._get_results()
    ...
    def save_result(self):
        if getattr(self, "_last_suite", None) in MVT_EXTENSION_TESTS:
            return
        if self._result_dir:
            file_name = f"{self.get_test_name()}_{self.webdriver.stb.profile}_result.json"
    ...
    def _open_and_press(self, page, keys, sleep_time=20, screenshot_suffix=None, delay=None):
        suite_url = f"{self.webdriver.stb.mvt_url}/{page}"
        self.logger.debug(f"Loading page: {suite_url}")

        self.webdriver.driver.get(suite_url)
        for key in keys:
            self.webdriver.stb.key_input(key)
            if delay:
                sleep(delay)
        sleep(sleep_time)
    ...
    def _load_suite(self, suite):
        ...
        if suite == "application-memory-test":
            suite_url = f"{self.webdriver.stb.mvt_url}/application_memory.html"
            self.logger.debug(f"Loading page: {suite_url}")
            self.webdriver.driver.get(suite_url)
            for i in range(1, 25):
                self.logger.debug(f"key press : {i}")
                self.webdriver.stb.key_input(KEY_OK)
                sleep(2)
                if self._browser_has_crashed():
                    if i > 19:
        ...
        suite_url = f"{self.webdriver.stb.mvt_url}/?test_type={suite}&profile={self.webdriver.stb.profile}&command=run"
        self.logger.debug(f"Load MVT test suite: {suite_url}")
        self.webdriver.driver.get(suite_url)
        self.webdriver.wait_until_element_visible("testlist")
        self.webdriver.is_js_loaded()

    def _browser_has_crashed(self):
        try:
            resp = self.webdriver.stb.shell("ps aux | grep WPEWebProcess | grep -v grep")
    ...
    def _get_results(self):
        self._current_test = int(self.webdriver.driver.execute_script("return globalRunner.currentTestIdx"))
        self._results = _fix_test_results_ver_type(self.webdriver.driver.execute_script("return getMvtTestResults()"))
        if self._result_dir:
            self.collect_screenshot()
            self.save_result()
...
@pytest.fixture
def mvt_remote_runner(test_logger, webdriver):
    runner = MVTRemoteRunner(test_logger, webdriver)
    yield runner
    runner.save_result()
